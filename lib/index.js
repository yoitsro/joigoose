...
internals.convert = joiObject => {
  ...
  if (joiObject.$_terms.metas.length > 0) {
    ...
    for (var i = 0; i < joiObject.$_terms.metas.length; i++) {
      ...
      for (var meta in joiObject.$_terms.metas[i]) {
        if (meta === "_mongoose") {
          for (var key in joiObject.$_terms.metas[i]._mongoose) {
            // Check for _objectId
            if (
              key === "type" &&
              joiObject.$_terms.metas[i]._mongoose[key] === "ObjectId"
            ) {
              ...
              output[key] = joiObject.$_terms.metas[i]._mongoose[key];
            }
          }
        }
      }
  ...
  if (joiObject.type === "array") {
    ...
    let schemaOptions = joiObject.$_terms.metas.reduce(
      (options, currentOption) => {
        return Object.assign({}, options._mongoose, currentOption._mongoose);
      },
      {}
    );
    ...
    for (var i = 0; i < joiObject.$_terms.metas.length; i++) {
      ...
      for (var meta in joiObject.$_terms.metas[i]) {
        if (meta === "_mongoose") {
          for (var key in joiObject.$_terms.metas[i]._mongoose) {
            // Check for _objectId
            if (key === "type" && joiObject.$_terms.metas[i]._mongoose[key] === "ObjectId") {
              return true;
            } else {
              return false;
            }
