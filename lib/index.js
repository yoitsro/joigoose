...
internals.root = (mongoose, options, subdocumentOptions, joi = Joi) => {
  Assert(mongoose);
...
internals.convert = (joiObject, options = {}) => {
  ...
  if (!internals.joi.isSchema(joiObject)) {
    joiObject = internals.joi.object(joiObject);
  }
  const output = {};
  ...
  if (joiObject.type === "object") {
    ...
    if (joiObject._flags.unknown) 
      output[typeKey] = internals.mongoose.Schema.Types.Mixed;
...
internals.mongooseValidateWrapper = async (originalJoiSchema, value) => {
  ...
  if (joiSchema._isObjectId) {
    joiSchema = internals.joi.alternatives(
      joiSchema,
      internals.joi.object().instance(internals.mongoose.Types.ObjectId)
    );
  }
